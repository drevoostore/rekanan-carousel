...
<body>
  ...
  <script>
    ...
    function renderCarousel(data) {
      ...
      const selected = data ? data : shuffled.slice(0, CARDS_PER_VISIT);
    ...
    let currentPosition = 0; let cardWidth = 0; let visibleCards = 4; let totalCards = 16; let maxPosition = Math.max(0, totalCards - visibleCards); let autoScrollInterval = null; let isDragging = false; let startX = 0; let currentTranslate = 0; let prevTranslate = 0; let isSearchMode = false; let searchTotalCards = 0;
    ...
    function resetAutoScroll() {
      ...
      const totalToScroll = isSearchMode ? searchTotalCards : totalCards;
      const searchMaxPosition = Math.max(0, totalToScroll - visibleCards);
      autoScrollInterval = setInterval(() => {
        console.log('AutoScroll tick:', currentPosition, 'max:', searchMaxPosition);
        if (currentPosition >= searchMaxPosition) currentPosition = 0;
        else currentPosition++;
        updateCarousel();
      }, 2000);
      console.log('AutoScroll started, interval:', 2000, 'searchMode:', isSearchMode);
    }
    function filterCards(query) {
      ...
      const matched = allRekanans.filter(r => {
        const kota = r.kota.toLowerCase();
        const nama = r.nama.toLowerCase();
        return kota.includes(q) || nama.includes(q);
      });
      if (matched.length === 0) {
